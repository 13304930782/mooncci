...
function unwrapSettings(input: any) {
...
export function normalizeHomeSettings(input: any): HomeSettings {
  const source = unwrapSettings(input);
  ...
  return {
    ...
    hero_chips: asStringArray(source.hero_chips ?? source.hero_chips_json, defaultHomeSettings.hero_chips),
    ...
    topics: asTopics(source.topics ?? source.topics_json, defaultHomeSettings.topics),
